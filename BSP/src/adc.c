...
void adc_init(void)
{
    ...
    channel_config.Channel      = ADC_CHANNEL_0;                   // 通道 0：LIGHT_PIN（PA0，光敏电阻）
    channel_config.Rank         = ADC_RANK_CHANNEL_NUMBER;         // Rank 与通道号一致
    channel_config.SamplingTime = ADC_SAMPLETIME_239CYCLES_5;      // 采样时间 239.5 个 ADC 周期
    if (HAL_ADC_ConfigChannel(&g_adc_handle, &channel_config) != HAL_OK)
    {
        Error_Handler();
    }

    channel_config.Channel      = ADC_CHANNEL_1;                    // 通道 1：NTC_PIN（PA1，NTC 热敏电阻）
    if (HAL_ADC_ConfigChannel(&g_adc_handle, &channel_config) != HAL_OK)
    {
        Error_Handler();
    }
...
void HAL_ADC_MspInit(ADC_HandleTypeDef* adcHandle)
{
    ...
    if(adcHandle->Instance==ADC1)
    {
        ...
        gpio_init_struct.Pin  = LIGHT_PIN|NTC_PIN;      // PA0: LIGHT_PIN（光敏），PA1: NTC_PIN（温度）
